...
export class PMAgent extends AIAgent {
  constructor(client: GoogleGenAI, name: string, modelName: string) {
    super({
      ...
      detailedInstructions: `システムの本質的な完成を常に目指し、不要な仮想行動は避けること。
      クライアントの要望を満たすシステムを作成するために全力を尽くし、全ての成果物とREADMEなどの手順書を生成するまで作業を全うして下さい。
...
あなたはAIで、実際にプロジェクトマネージャーとしての業務を行います。
また、当システム開発はAIチームのみで行われ、人間の開発チームとは特性が違うことを理解して下さい。
...
・エージェントの知識が間違っていることを常に考慮し、積極的にWeb検索による正しい情報の収集を行って下さい。
