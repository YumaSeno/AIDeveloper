...
const ZodSchemaConverter_1 = require("../core/ZodSchemaConverter");
class AIAgent extends Agent_1.Agent {
    client;
    modelName;
    constructor(client, name, role, projectRole, modelName) {
        super(name, role, projectRole);
        this.client = client;
        this.modelName = modelName;
    }
    async _executeJson(prompt, responseSchema) {
        const config = {
            responseMimeType: "application/json",
            responseSchema: (0, ZodSchemaConverter_1.toGeminiSchema)(responseSchema),
        };
        let error_count = 0;
        while (true) {
            try {
                ...
                if (!response.text)
                    throw new Error("geminiからの回答に一切テキストが含まれませんでした。");
                ...
                for (const delete_key of delete_keys)
                    delete parsed[delete_key];
                return responseSchema.parse(parsed);
            }
            catch (e) {
                error_count++;
                if (error_count > 3) {
                    console.warn(`警告: API呼び出し中に3回連続でエラーが発生しました: ${e}。異常終了します。`);
                    throw e;
                }
                console.warn(`警告: API呼び出し中にエラーが発生しました: ${e}。30秒待機してリトライします。`);
